...
package object games {
  // Game dependent data "describing" the move for use by UI, logging, etc.
  type Move = Any
  /**
    * Identifies the players.
    */
  sealed abstract class Token() {
    def opponent: Token = this match {
      case P1 => P2
      case P2 => P1
    }
  }
  ...
  trait Player[G <: Game] {
    ...
    /**
      * Must return a move unless the game is over.
      */
    def move(g: G): Option[(G, Move)]
  }
  /**
  ...
  trait Game {
    /**
      * The player with turn in hand for this game state. Not defined if the game is completed.
      */
    def turnInHand: Token
    /**
      * Left(token or draw) or Right(available moves).
      * If the game continues (the Right option) the list must not be empty but, rather,
      * represent a pass.
      */
    def result: Either[Option[Token], List[(Game, Any)]]
  }
  /*
    object Game {
      /**
